...
import { tenantUrl, portalUrl } from "@/lib/url";
    ...
    <form action={formAction} className="space-y-4">
      ...
      <div className="rounded-md border border-border bg-muted/40 px-3 py-2.5 space-y-1.5 text-sm">
        <div className="flex gap-3">
          ...
          <span className="font-mono text-xs">{tenantUrl(displaySlug)}</span>
        </div>
        <div className="flex gap-3">
          <span className="text-muted-foreground w-24 shrink-0">Client portal</span>
          <span className="font-mono text-xs">{portalUrl(displaySlug)}</span>
        </div>
      </div>
